...
RT INTO course (course_id, course_name)
VALUES 
...
course_name    | student_name | grade 
------------------+--------------+-------
 Chemistry        | Dilnoza      |   4.5
 Physics          | Lola         |  4.55
 Biology          | Dilnoza      |  4.73
 Computer Science | Jasur        |  4.15
 Mathematics      | Jasur        |   4.6
...
select c.course_name, avg(g.grade) as student_avg from student_course as sc
right join course as c on c.course_id = sc.course_id
left join grade as g on g.student_course_id = sc.student_course_id
group by c.course_name
order by student_avg desc;
   course_name    |    student_avg     
------------------+--------------------
 Chemistry        |             4.1175
 Physics          |              4.085
 Biology          |               4.06
 Mathematics      | 3.8137499999999998
 Computer Science |             3.7675
...
select min(s.age) as student_age, c.course_name 
from student_course as sc 
right join student as s on sc.student_id = s.student_id
left join course as c on sc.course_id = c.course_id
group by c.course_name
having student_age != s.age;
---------------------------------------------------
student_age |   course_name    
-------------+------------------
          19 | 
          19 | Computer Science
          19 | Chemistry
...
select c.course_name, avg(g.grade) as the_best_course from student_course as sc
right join course as c on c.course_id = sc.course_id
left join grade as g on g.student_course_id = sc.student_course_id
group by c.course_name
order by the_best_course desc
limit 1;

course_name | the_best_course 
-------------+-----------------
 Chemistry   |          4.1175
